...
namespace
{
    constexpr size_t KEY_SIZE = 44;      // 32 key + 12 nonce
    ...
    const AlgorithmInfo algorithm_info{ "ChaCha20", KEY_SIZE };
    ...
    public:
        ChaCha20(const uint8_t* key, const uint8_t* nonce)
        {
            ...
            for (int i = 0; i < 8; ++i)
                state[4 + i] = reinterpret_cast<const uint32_t*>(key)[i];

            state[12] = 0;
            state[13] = 0;

            state[14] = reinterpret_cast<const uint32_t*>(nonce)[0];
            state[15] = reinterpret_cast<const uint32_t*>(nonce)[1];
        }
...
extern "C"
{
    ...
    int encrypt(ConstBuffer key, ConstBuffer input, MutBuffer* output)
    {
        try
        {
            ...
            const uint8_t* secretKey = key.data;
            const uint8_t* nonce = key.data + 32;
