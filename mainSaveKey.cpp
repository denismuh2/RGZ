...
int main()
{
    ...
    while (true)
    {
        ...
        else if (choice == 7)  // Load key
        {
            std::string path;
            std::cout << "Load key from file: ";
            std::getline(std::cin, path);

            std::vector<uint8_t> loaded_key;
            if (!load_binary(path, loaded_key))
            {
                std::cout << "Failed to load key\n";
                continue;
            }

            if (loaded_key.size() != info->key_size)
            {
                std::cout << "Invalid key size. Expected " << info->key_size
                          << " bytes, got " << loaded_key.size() << " bytes\n";
                continue;
            }
